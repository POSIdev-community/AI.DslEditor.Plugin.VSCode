...
JSA DSL — плагин для VSCode, который помогает при написании пользовательских [DSL-правил](), обеспечивая полноценную подсветку синтаксиса JSA DSL, интеллектуальное автодополнение и комплексную диагностику кода. Дополнительно плагин поддерживает быстрые исправления ошибок, навигацию по коду и автоматическое форматирование при изменении файлов `.jsa.dsl`.
...
* `jsa.dsl.languageServer.path` — пользовательский путь к языковому серверу. По умолчанию не задан, так как используется встроенный сервер.
